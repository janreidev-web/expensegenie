...
// --- CHANGE: StickyRobot now correctly uses isFooterVisible ---
function StickyRobot({ currentSectionIndex, sectionMessages, isFooterVisible }) {
    ...
    return (
        <div
            className={`fixed bottom-4 right-4 sm:bottom-8 sm:right-8 z-40 transition-all duration-700 ease-in-out
...
function KeyFeaturesSummary({ onVisibleChange }) {
    ...
    return (
        <div ref={ref} className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-teal-50 to-blue-100 py-16 px-6 sm:py-24 sm:px-8 text-center snap-start">
            ...
            {/* --- CHANGE: Reverted to a grid, now with 2 columns on mobile and 4 on large screens --- */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap- max-w-7xl mx-auto">
                {features.map((feature, index) => (
                    // --- CHANGE: Removed carousel-specific classes (w-5/6, flex-shrink-0, snap-center) ---
                    <div
                        key={index}
...
// --- CHANGE: isFooterVisible is now a prop ---
function LandingPage({ isFooterVisible }) {
    ...
    // --- CHANGE: Removed the container with h-screen and overflow-y-auto ---
    return (
